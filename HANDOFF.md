...
_Stand: 2026-05-20 abends — v2.3.178 fertig, **ETF150 AE-Lock im Code**, nächster Praxistest offen_

---

## 2026-05-20 — was passiert ist

Ausgangslage: BEFUND `sessions/2026-05-19_test/BEFUND.md` zeigte SNR ~1:100–250
(Loch ≈ 17 px vs. Drift ≈ 39 000 px in 5 s Referenz-Alter). Hauptkiller:
Auto-Belichtung jagt + verlustbehaftetes RTSP. Empfehlung war: UVC (SV205+SV28).

**Berts Wunsch:** ETF150 weiter nutzen, falls möglich. Daraufhin systematische
Untersuchung der `/roc/isp/param`-Firmware-Endpoint (PUT geht ohne Auth, GET ist
403). Schema-Raum durch Blind-PUT + Snapshot-Messung abgetastet.

### Befund: `{"ae":{"enable":0}}` auf `/roc/isp/param` lockt AE deterministisch
**Provokationstest 2026-05-20** (Hand-Stimulus): Mit Lock aktiv stürzt der
Mittelwert beim Linsen-Abdecken von 130 auf **31 graylevel** und steht **8 s lang stabil**
bei 31 — keine AE-Kompensation. Beim Aufdecken sofortige Sättigung auf 255.
Klassische Lock-Signatur. Codex' frühere Skepsis ("könnte Drift sein") damit
widerlegt — der Stimulus ist viel größer als jede natürliche Drift.

### Recovery-Schema-Hunt (8 Kandidaten)
**CLEAN-Recoveries** (delta vs Baseline < 3 grays):
- `{"ae":{"enable":1,"mode":1}}` → +1,14 ✓ **(in APK verwendet)**
- `{"ae":{"enable":1},"exposure":{"auto":1}}` → −1,20
- `{"exposure":{"mode":"auto"}}` → −1,48

### v2.3.178 Code-Stand (im Working-Tree, NICHT committed/getaggt)
- `index.html` (`applyEtfMode`): PUT `/roc/isp/param {"ae":{"enable":0}}` zusätzlich
  zum bestehenden OSD-Off-PUT, sobald ETF150-Stream aktiv wird (Zeile 1463-1468)
- `index.html` (`btnRtspStop` handler): PUT `{"ae":{"enable":1,"mode":1}}` vor
  `stopRtsp()`, conditional auf `camIsEtf` (Zeile 4609-4620)
- Versions-Bump 2.3.177 → 2.3.178, versionCode 181 → 182, alle 6 Versions-Quellen
  synchron (check_versions.sh = ✓ ALLES SYNCHRON)
- assets-Mirror + G:\Meine Ablage\Claude\index.html aktualisiert

### Open / nächste Schritte
- **Code reviewen + committen + Tag `v2.3.178`** → GitHub Actions baut APK, dann
  `gh release download v2.3.178 --pattern "*.apk"` + `adb install -r`
- **Praxistest am Stand**: gleiche Disziplin (KK25/GK25), `pts_out_of_range`-Flut
  vs. v2.3.177-Daten vergleichen → quantifiziert Wirkung von AE-Lock allein.
- **Wenn AE-Lock allein nicht reicht** (SNR-Headroom immer noch zu knapp):
  Codec-Noise + Auflösung sind die nächsten Hebel, dann doch UVC (SV205+SV28).

### Caveats / Lessons
- **PUTs auf `/roc/isp/param` können Cam in defekten Zustand setzen** — bewiesen
  durch `{"exposure":{"value":{"@def":60000}}}`-Brick (graues Bild bis Power-Cycle).
  Deshalb in der APK **nur die zwei verifizierten Schemas** (lock/unlock), keine
  weiteren Experimente in Production-Code.
- **Cam quittiert alle JSON-PUTs identisch** mit `statusCode:0 OK leave` — kein
  Schema-Validierungs-Feedback. Wirkung nur visuell/messtechnisch prüfbar.
- **Recovery `{"ae":{"enable":1,"mode":1}}` ist CLEAN bei stabiler Szene**, war
  aber in der früheren Zeitreihe nach starkem Stimulus unsauber (+14 offset).
  Power-Cycle bleibt die deterministische Reset-Methode wenn nötig.

### Session-Daten dieser Untersuchung
- `sessions/exposure_lock/run_safe_*/` — erste safe-Variante (n=3)
- `sessions/exposure_lock/provocation_*/` — Provokationstest (Smoking Gun: 31→255)
- `sessions/exposure_lock/recovery_hunt_*/` — 8-Kandidaten-Hunt
- `sessions/exposure_lock/persistence_*/` — 3-Min-Persistenz-Test (Stand 20:xx)

---

_Stand davor (2026-05-13): v2.3.170, 2. Praxistest am 2026-05-14_
